...
public class TimeSheetServiceImpl implements TimeSheetService {
    ...
    @Override
    public List<Day> getBetween(LocalDate from, LocalDate to, Jwt jwt) throws ODataException {
        List<TimeSheetEntry> retrievedEntries = timeSheetRepository
                .getByDateAndUser(from, to, authenticationFacade.getLoggedUser(jwt).getId());
    ...
    @Override
    public List<TimeSheetEntry> createOrUpdate(List<Day> days, Jwt jwt) {
        final List<TimeSheetEntry> entries = dayService.split(days, authenticationFacade.getLoggedUser(jwt).getId());
