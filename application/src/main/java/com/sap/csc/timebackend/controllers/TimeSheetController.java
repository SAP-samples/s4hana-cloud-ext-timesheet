...
public class TimeSheetController {
    ...
    public ResponseEntity<List<Day>> getDays(
            ...
            @RequestParam @DateTimeFormat(pattern = Constants.DD_MM_YYYY) LocalDate dateTo,
            @AuthenticationPrincipal Jwt jwt) throws ODataException {

        validateDate(dateFrom, dateTo);
        return ResponseEntity.ok(timeSheetService.getBetween(dateFrom, dateTo, jwt));
    ...
    @PostMapping
    public ResponseEntity<List<TimeSheetEntry>> createOrUpdate(@RequestBody List<Day> days, @AuthenticationPrincipal Jwt jwt) {
        ...
        return ResponseEntity.accepted().body(timeSheetService.createOrUpdate(days, jwt));
    }
